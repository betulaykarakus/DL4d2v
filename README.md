...
通过本项目的代码，可以训练并生成导出word2vec和doc2vec的模型。并利用模型进行一系列的操作。
...
# 程序主要思路
1. 利用爬虫下载文章到数据库中  
2. 从数据库中把文章读到文件当中并加入分句  
3. 利用jieba分词对文章进行分词处理  
4. 磨合分词字典  
5. 利用gensim训练word2vec和doc2vec模型  
...
（5）有一些尝试性的helper functions，留在makecall.py中，但最后不需要调用。
# 二、用法示例 DEMOS
### Demo 0: 给文章分句及分词
+ 例 文章如下：
